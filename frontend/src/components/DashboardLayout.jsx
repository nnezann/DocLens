...
const NAV_ITEMS = [
  ...
  { label: 'Cases', icon: Folder, to: '/dashboard/investigations' },
  { label: 'Requests', icon: Tag, to: '/dashboard/requests' },
  { label: 'Reports', icon: Calendar, to: '/dashboard/reports' },
]

function SidebarLink({ label, icon: Icon, to }) {
  return (
    <NavLink
      to={to}
      end={to === '/dashboard'}
      className={({ isActive }) =>
        `flex w-full items-center gap-3 rounded-xl px-4 py-3 text-[15px] font-medium transition ${
          isActive ? 'bg-slate-100 text-ink-900' : 'text-slate-500 hover:bg-slate-50 hover:text-ink-900'
        }`
      }
    >
    ...
    <div className="flex min-h-screen bg-white">
      ...
      <aside className="flex w-64 shrink-0 flex-col justify-between border-r border-slate-100 px-5 py-6">
        <div>
          ...
          >
            <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M6 20 L18 5" stroke="#131A2C" strokeWidth="2.4" strokeLinecap="round" />
        ...
        <div className="space-y-1">
          <SidebarLink label="Settings" icon={Settings} to="/dashboard/settings" />
          <button
            type="button"
